package edu.aku.hassannaqvi.mapps_form_l1.sync;
...
public class SyncEnroll extends AsyncTask<Void, Void, String> {

    private static final String TAG = "SyncEligibles";
    private Context mContext;
    private ProgressDialog pd;

    public SyncEnroll(Context context) {
        mContext = context;
    }
        ...
        try {
            ...
            for (int i = 0; i < json.length(); i++) {
                ...
                if (jsonObject.getString("status").equals("1")) {
                    db.updateEnroll(jsonObject.getString("id"));
                    sSynced++;
                }
            }
            Toast.makeText(mContext, sSynced + " Enroll synced." + String.valueOf(json.length() - sSynced) + " Errors.", Toast.LENGTH_SHORT).show();

            pd.setMessage(sSynced + " Enroll synced." + String.valueOf(json.length() - sSynced) + " Errors.");
            pd.setTitle("Done uploading Enroll data");
            pd.show();
            ...
            pd.setTitle("Enroll's Sync Failed");
            pd.show();
        }
    }
